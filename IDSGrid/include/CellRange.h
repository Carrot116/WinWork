...
//
// MFC Grid Control - interface for the CIDSCellRange class.
//
...
//
// For use with CXTPGridCtrl v2.20+
//
//////////////////////////////////////////////////////////////////////
...
// Operations
public:
    explicit CIDSCellID(int nRow = -1, int nCol = -1) : row(nRow), col(nCol) {}

    int IsValid() const { return (row >= 0 && col >= 0); }
    int operator==(const CIDSCellID& rhs) const { return (row == rhs.row && col == rhs.col); }
    int operator!=(const CIDSCellID& rhs) const { return !operator==(rhs); }
};
...
public:
    ...
    CIDSCellRange(int nMinRow = -1, int nMinCol = -1, int nMaxRow = -1, int nMaxCol = -1)
    {
    ...
    int  InRange(const CIDSCellID& cellID) const;
    int  Count() { return (m_nMaxRow - m_nMinRow + 1) * (m_nMaxCol - m_nMinCol + 1); }
    
    CIDSCellID  GetTopLeft() const;
    CIDSCellRange  Intersect(const CIDSCellRange& rhs) const;
    ...
    void operator=(const CIDSCellRange& rhs);
    int  operator==(const CIDSCellRange& rhs);
    int  operator!=(const CIDSCellRange& rhs);
...
inline void CIDSCellRange::Set(int minRow, int minCol, int maxRow, int maxCol)
{
     m_nMinRow = minRow;
     m_nMinCol = minCol;
     m_nMaxRow = maxRow;
...
inline void CIDSCellRange::operator=(const CIDSCellRange& rhs)
{
    if (this != &rhs) Set(rhs.m_nMinRow, rhs.m_nMinCol, rhs.m_nMaxRow, rhs.m_nMaxCol);
}

inline int CIDSCellRange::operator==(const CIDSCellRange& rhs)
{
...
inline int CIDSCellRange::IsValid() const
{
     return (m_nMinRow >= 0 && m_nMinCol >= 0 && m_nMaxRow >= 0 && m_nMaxCol >= 0 &&
             m_nMinRow <= m_nMaxRow && m_nMinCol <= m_nMaxCol);
}

inline int CIDSCellRange::InRange(int row, int col) const
{
     return (row >= m_nMinRow && row <= m_nMaxRow && col >= m_nMinCol && col <= m_nMaxCol);
}

inline int CIDSCellRange::InRange(const CIDSCellID& cellID) const
{
     return InRange(cellID.row, cellID.col);
}

inline CIDSCellID CIDSCellRange::GetTopLeft() const
{
     return CIDSCellID(m_nMinRow, m_nMinCol);
}

inline CIDSCellRange CIDSCellRange::Intersect(const CIDSCellRange& rhs) const
{
     return CIDSCellRange(max(m_nMinRow,rhs.m_nMinRow), max(m_nMinCol,rhs.m_nMinCol),
