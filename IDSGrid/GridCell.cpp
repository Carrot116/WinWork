...
// Ken Bertelson - 12 Apr 2000 - Split CIDSGridCell into CIDSGridCell and CIDSGridCellBase
...
IMPLEMENT_DYNCREATE(CIDSGridCell, CIDSGridCellBase)
IMPLEMENT_DYNCREATE(CGridDefaultCell, CIDSGridCell)

/////////////////////////////////////////////////////////////////////////////
// GridCell 
tagFontInfo g_tFontInfo = 
{	
	-12, 0, _T("ËÎÌו")
};
CIDSGridCell::CIDSGridCell()
{
    m_plfFont = NULL;
	CIDSGridCell::Reset();
}
...
void CIDSGridCell::operator=(const CIDSGridCell& cell)
{
    if (this != &cell) CIDSGridCellBase::operator=(cell);
}

void CIDSGridCell::Reset()
{
    CIDSGridCellBase::Reset();
...
void  CIDSGridCell::SetFontEsay(int nFontHeight, LPCTSTR pszFontName, BOOL bBold)
{	
	LOGFONT lf;
...
void CIDSGridCell::SetFont(const LOGFONT* plf)
{
    if (plf == NULL)
    {
        delete m_plfFont;
...
LOGFONT* CIDSGridCell::GetFont() const
{
    if (m_plfFont == NULL)
    {
...
CFont* CIDSGridCell::GetFontObject() const
{
...
DWORD CIDSGridCell::GetFormat() const
{
...
UINT CIDSGridCell::GetMargin() const           
{
    if (m_nMargin == (UINT)-1)
    {
        CGridDefaultCell *pDefaultCell = (CGridDefaultCell*) GetDefaultCell();
        ...
        int nMargin = pDefaultCell->GetMargin();
		if(-1 == nMargin)
		{
			nMargin = 3;
		}
 ...
 UINT  CIDSGridCell::GetVMargin() const
 {
	 if (m_nVMargin == (UINT)-1)
	 {
		 CGridDefaultCell *pDefaultCell = (CGridDefaultCell*) GetDefaultCell();
		 if (NULL == pDefaultCell)
		 {
		 ...
		 int nMargin = pDefaultCell->GetVMargin();
		 if(-1 == nMargin)
		 {
			 nMargin = 2;
		 }
...
BOOL CIDSGridCell::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
    if ( m_bEditing )
	{      
        if (m_pEditWnd)
	...
	{   
		...
		m_pEditWnd = new CIDSInPlaceEdit(pGrid, rect, dwStyle, nID, nRow, nCol, GetText(), nChar);
    }
    return TRUE;
}

void CIDSGridCell::EndEdit()
{
    if (m_pEditWnd)
        ((CIDSInPlaceEdit*)m_pEditWnd)->EndEdit();
}

void CIDSGridCell::OnEndEdit()
{
    m_bEditing = FALSE;
    m_pEditWnd = NULL;
}
...
CGridDefaultCell::CGridDefaultCell() 
{
...
#else
    m_nFormat = DT_CENTER;//DT_LEFT|DT_VCENTER|DT_SINGLELINE|DT_NOPREFIX | DT_END_ELLIPSIS;
#endif
	...
	LOGFONT lf;
	memset(&lf,0,sizeof(LOGFONT));
	_tcscpy(lf.lfFaceName,  (LPCTSTR)g_tFontInfo.szFontName);
	//lf.lfWeight			= FW_ULTRABOLD;
	lf.lfHeight			= g_tFontInfo.nFontHeight;
	lf.lfWeight			= g_tFontInfo.nFontWight;
	lf.lfItalic			= FALSE;
	lf.lfOrientation	= 0;
	lf.lfEscapement		= 0;
	lf.lfUnderline		= FALSE;
...
void CGridDefaultCell::SetFont(const LOGFONT* plf)
{
    ...
    if (pDC)
    {
		...
		CSize cTextSize = pDC->GetTextExtent(_T(" "), 1);
        SetMargin(cTextSize.cx);
