...
//
// For use with CIDSGridCtrl v2.20+
//
//////////////////////////////////////////////////////////////////////
...
//#include "NHScrollBarEx.h"
#include <ScrollBarEx.h>
#ifdef IDS_GRID_DLL
	#define IDS_GRID_API  __declspec(dllexport)
#else
	#define IDS_GRID_API  __declspec(dllimport)
#endif 

typedef void (*UserDraw)(CDC * pDC, DWORD dwContext);
///////////////////////////////////////////////////////////////////////////////////
...
typedef CTypedPtrArray<CObArray, CIDSGridCellBase*> GRID_ROW;
...
#define WM_RGRIDLBUTTONDOWN        (WM_USER+100 )
#define WM_RGRIDLBUTTONUP        (WM_USER+101 )
#define WM_RGRIDRBUTTONDOWN        (WM_USER+102 )
#define WM_RGRIDRBUTTONUP        (WM_USER+103 )

#define WM_GRID_FOCUSCELL_CHANGE      (WM_USER+110 )

#define  WM_GRID_VSCOLL			(WM_USER+120)
#define  WM_GRID_HSCOLL			(WM_USER+121)
...
class IDS_GRID_API  CIDSGridCtrl : public CWnd
{
    DECLARE_DYNCREATE(CIDSGridCtrl)
    friend class CIDSGridCell;
    friend class CIDSGridCellBase;

// Construction
public:
    CIDSGridCtrl(BOOL bAutoScroll = TRUE, int nRows = 0, int nCols = 0, int nFixedRows = 0, int nFixedCols = 0);
...
public:
    ...
    BOOL SetRowHeight(int row, int height, BOOL bResetScrollBars = TRUE);
    int  GetColumnWidth(int nCol) const;
    BOOL SetColumnWidth(int col, int width, BOOL bResetScrollBars = TRUE);

    BOOL GetCellOrigin(int nRow, int nCol, LPPOINT p);
    BOOL GetCellOrigin(const CIDSCellID& cell, LPPOINT p);
    BOOL GetCellRect(int nRow, int nCol, LPRECT pRect);
    BOOL GetCellRect(const CIDSCellID& cell, LPRECT pRect);

    BOOL GetTextRect(const CIDSCellID& cell, LPRECT pRect);
    BOOL GetTextRect(int nRow, int nCol, LPRECT pRect);

    CIDSCellID GetCellFromPt(CPoint point, BOOL bAllowFixedCellCheck = TRUE);
	...
	void     SetDefCellVMargin( int nMargin)  { m_cellDefault.SetVMargin(nMargin); 
												m_cellFixedRowDef.SetVMargin(nMargin); 
												m_cellFixedColDef.SetVMargin(nMargin); 
												m_cellFixedRowColDef.SetVMargin(nMargin); }
	int      GetDefCellVMargin() const        { return m_cellDefault.GetVMargin();     }
	...
	LOGFONT*      GetDefCellFont() const         { return m_cellDefault.GetFont();     }
	void		 SetDefCellFont(LOGFONT* pLogFont)    { m_cellDefault.SetFont(pLogFont); 
													m_cellFixedRowDef.SetFont(pLogFont); 
													m_cellFixedColDef.SetFont(pLogFont); 
													m_cellFixedRowColDef.SetFont(pLogFont); }
    // ***************************************************************************** //

    int GetSelectedCount() const                  { return (int)m_SelectedCellMap.GetCount(); }

    CIDSCellID SetFocusCell(CIDSCellID cell);
    CIDSCellID SetFocusCell(int nRow, int nCol);
    CIDSCellID GetFocusCell() const                  { return m_idCurrentCell;           }
...
///////////////////////////////////////////////////////////////////////////////////
public:
    CIDSGridCellBase* GetDefaultCell(BOOL bFixedRow, BOOL bFixedCol) const;

///////////////////////////////////////////////////////////////////////////////////
// Grid cell Attributes
///////////////////////////////////////////////////////////////////////////////////
public:
    CIDSGridCellBase* GetCell(int nRow, int nCol) const;   // Get the actual cell!
    ...
    //  CIDSGridCellBase-derived class's GetText() to accomplish same thing
	...
	BOOL SetItemFont(int nRow ,int nCol, int nFontHeight, LPCTSTR pszFontName = _T("ËÎÌו"), BOOL bBold = FALSE);
...
public:
	...
	void ClearCells(CIDSCellRange Selection);
    ...
    void EnsureVisible(CIDSCellID &cell)       { EnsureVisible(cell.row, cell.col); }
    void EnsureVisible(int nRow, int nCol);
    BOOL IsCellVisible(int nRow, int nCol);
    BOOL IsCellVisible(CIDSCellID cell);
    BOOL IsCellEditable(int nRow, int nCol) const;
    BOOL IsCellEditable(CIDSCellID &cell) const;
    BOOL IsCellSelected(int nRow, int nCol) const;
    BOOL IsCellSelected(CIDSCellID &cell) const;
    ...
    BOOL RedrawCell(const CIDSCellID& cell, CDC* pDC = NULL);
...
///////////////////////////////////////////////////////////////////////////////////
    ...
    virtual BOOL PasteTextToGrid(CIDSCellID cell, COleDataObject* pDataObject, BOOL bSelectPastedCells=TRUE);
#endif
...
///////////////////////////////////////////////////////////////////////////////////
public:
    CIDSCellID GetNextItem(CIDSCellID& cell, int nFlags) const;
	...
	void SetCompareFunction(PFNLVCOMPARE pfnCompare, DWORD dwSortContext);
...
protected:
    ...
    LRESULT SendCacheHintToParent(const CIDSCellRange& range) const;

    BOOL InvalidateCellRect(const int row, const int col);
    BOOL InvalidateCellRect(const CIDSCellID& cell);
    BOOL InvalidateCellRect(const CIDSCellRange& cellRange);
    void EraseBkgnd(CDC* pDC);

    BOOL GetCellRangeRect(const CIDSCellRange& cellRange, LPRECT lpRect);

    BOOL SetCell(int nRow, int nCol, CIDSGridCellBase* pCell);
...
public:
    CIDSCellID GetTopleftNonFixedCell(BOOL bForceRecalculation = FALSE);
    CIDSCellRange GetUnobstructedNonFixedCellRange(BOOL bForceRecalculation = FALSE);
    CIDSCellRange GetVisibleNonFixedCellRange(LPRECT pRect = NULL, BOOL bForceRecalculation = FALSE);
    CIDSCellRange GetVisibleFixedCellRange(LPRECT pRect = NULL, BOOL bForceRecalculation = FALSE);
...
protected:
    ...
    // CIDSGridCellBase Creation and Cleanup
    virtual CIDSGridCellBase* CreateCell(int nRow, int nCol);
...
protected:
    ...
    CIDSCellID     m_LeftClickDownCell, m_SelectionStartCell;
    CIDSCellID     m_idCurrentCell, m_idTopLeftCell;
    INT_PTR     m_nTimerID;
    int         m_nTimerInterval;
    int         m_nResizeCaptureRange;
    BOOL        m_bAllowRowResize, m_bAllowColumnResize;
    int         m_nRowsPerWheelNotch;
    CMap<DWORD,DWORD, CIDSCellID, CIDSCellID&> m_SelectedCellMap, m_PrevSelectedCellMap;

#ifndef GRIDCONTROL_NO_TITLETIPS
    CIDSTitleTip   m_TitleTip;             // Title tips for cells
#endif

    // Drag and drop
    CIDSCellID     m_LastDragOverCell;
#ifndef GRIDCONTROL_NO_DRAGDROP
    CIDSGridDropTarget m_DropTarget;       // OLE Drop target for the grid
#endif
...
protected:
    void SelectAllCells();
    void SelectColumns(CIDSCellID currentCell, BOOL bForceRedraw=FALSE, BOOL bSelectCells=TRUE);
    void SelectRows(CIDSCellID currentCell, BOOL bForceRedraw=FALSE, BOOL bSelectCells=TRUE);
    void SelectCells(CIDSCellID currentCell, BOOL bForceRedraw=FALSE, BOOL bSelectCells=TRUE);
    void OnSelecting(const CIDSCellID& currentCell);

    // Generated message map functions
    //{{AFX_MSG(CIDSGridCtrl)
...
private:	
	...
	COLORREF m_clFixtGridLine;
	COLORREF m_clSelectBK;
	BOOL m_bDrawGridLine;
	BOOL m_bAutoShowScrollBar;
	BOOL m_bHasHScrollBar;
	BOOL m_bHasVScrollBar;
	BOOL m_bDrawFocusCell;
	UserDraw m_pBeforeDraw;
	DWORD m_dwBeforeDrawContext;
	BOOL m_bEnableSelectCol; 
	UserDraw m_pAfterDraw;
	DWORD m_dwAfterDrawContext;


	DWORD m_dwSortContext;
	
	CScrollBarEx m_HScroll;
	CScrollBarEx m_VScroll;
public:
	void SetEnableSelectCol(BOOL bEnable);
	void SetRowFullSelect(BOOL bFullSelect);
	void SetDrawGridLine(BOOL bDrawGridLine);
	void SetHasHScrollBar(BOOL bHasHScrollBar);
	void SetHasVScroolBar(BOOL bHasVScrollBar);
	void SetAutoShowScrollBar(BOOL bAutoShowScrollBar);
	void SetDrawFocusCell(BOOL bDrawFocusCell);
	BOOL IsDrawFocusCell();
	BOOL IsDrawGridLine();
	void SetFixedGridLineColor(COLORREF clFixtGridLine);
	COLORREF GetFixtedGridLineColor();
	void SetSelectItemBKColor(COLORREF clSelBK);
	COLORREF GetSelectItemBKColor();
	void SetBeforeDrawFunc(UserDraw pBeforeDrawFun, DWORD dwBeforeContext);
	void SetAfterDrawFunc(UserDraw pAfterDrawFun, DWORD dwAfterContext);
	BOOL SetSelectRow(int nRow);
	void ScrollBarToZero(BOOL bNeedResetScrollBar);
...
inline CIDSGridCellBase* CIDSGridCtrl::GetDefaultCell(BOOL bFixedRow, BOOL bFixedCol) const
{ 
    if (bFixedRow && bFixedCol) return (CIDSGridCellBase*) &m_cellFixedRowColDef;
    if (bFixedRow)              return (CIDSGridCellBase*) &m_cellFixedRowDef;
    if (bFixedCol)              return (CIDSGridCellBase*) &m_cellFixedColDef;
    return (CIDSGridCellBase*) &m_cellDefault;
}

inline CIDSGridCellBase* CIDSGridCtrl::GetCell(int nRow, int nCol) const
{
    ...
    if (GetVirtualMode())
    {
        CIDSGridCellBase* pCell = GetDefaultCell(nRow < m_nFixedRows, nCol < m_nFixedCols);
        ...
        if (GetFocusCell() == CIDSCellID(nRow, nCol)) gvdi.item.nState |= GVIS_FOCUSED;
		if(!m_InDestructor)
		{
        ...
        cell.SetGrid((CIDSGridCtrl*)this);

        return (CIDSGridCellBase*) &cell;
    }
...
inline BOOL CIDSGridCtrl::SetCell(int nRow, int nCol, CIDSGridCellBase* pCell)
{
    if (GetVirtualMode())
        return FALSE;
