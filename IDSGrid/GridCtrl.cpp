...
#pragma message(" -- CIDSGridCtrl: No titletips for cells with large data")
#endif
#ifdef GRIDCONTROL_NO_DRAGDROP
#pragma message(" -- CIDSGridCtrl: No OLE drag and drop")
#endif
#ifdef GRIDCONTROL_NO_CLIPBOARD
#pragma message(" -- CIDSGridCtrl: No clipboard support")
#endif
#ifdef GRIDCONTROL_NO_PRINTING
#pragma message(" -- CIDSGridCtrl: No printing support")
#endif


IMPLEMENT_DYNCREATE(CIDSGridCtrl, CWnd)
...
CIDSGridCtrl::CIDSGridCtrl(BOOL bAutoScroll, int nRows, int nCols, int nFixedRows, int nFixedCols )
{
	...
	m_pRtcDefault = RUNTIME_CLASS(CIDSGridCell);
	...
	ResetCustomize();
	m_bAutoShowScrollBar = bAutoScroll;
}

#if 0
void WriteLog(LPSTR pszText)
{
	OutputDebugStringA(pszText);
	CString strLogFile =  _T("E:\\IDS\\08-proj\\IDS\\ReleaseU\\Config\\IDSLog.txt");
	try
	{
		CFile cLogFile;
		cLogFile.Open((LPCTSTR)strLogFile, CFile::modeCreate | CFile::modeReadWrite | CFile::modeNoTruncate);
		cLogFile.SeekToEnd();
		cLogFile.Write(pszText, strlen(pszText)); 
		cLogFile.Flush();
		cLogFile.Close();
	}
...
BOOL CIDSGridCtrl::RegisterWindowClass()
{
	WNDCLASS wndcls;
...
BOOL CIDSGridCtrl::Initialise()
{
	...
	if (::IsWindow(m_hWnd))
	{
		//@@@@by wu.s.h  ModifyStyleEx(0, WS_EX_CLIENTEDGE);
	}
...
BOOL CIDSGridCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle)
{
	//AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
void CIDSGridCtrl::SetupDefaultCells()
{
	...
	m_cellDefault.SetMargin(2);
	m_cellDefault.SetVMargin(2);
}

void CIDSGridCtrl::PreSubclassWindow()
{
...
BOOL CIDSGridCtrl::PreTranslateMessage(MSG* pMsg) 
{
...
LRESULT CIDSGridCtrl::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	// TODO: Add your specialized code here and/or call the base class
	switch(message)
	{
...
LRESULT CIDSGridCtrl::SendMessageToParent(int nRow, int nCol, int nMessage) const
{
	if (!IsWindow(m_hWnd))
		return 0;
	...
	{
		HRESULT hRet = pOwner->SendMessage(WM_NOTIFY, nmgv.hdr.idFrom, (LPARAM)&nmgv);
		return hRet;
	}
	else
		return 0;
...
LRESULT CIDSGridCtrl::SendDisplayRequestToParent(GV_DISPINFO* pDisplayInfo) const
{
	if (!IsWindow(m_hWnd))
		return 0;
...
LRESULT CIDSGridCtrl::SendCacheHintToParent(const CIDSCellRange& range) const
{
	if (!IsWindow(m_hWnd))
		return 0;
...
int CIDSGridCtrl::GetLayer(int** pLayer) // used to save and restore order of columns
...
void CIDSGridCtrl::SetLayer(int* pLayer)
...
BEGIN_MESSAGE_MAP(CIDSGridCtrl, CWnd)
	//EFW - Added ON_WM_RBUTTONUP
	//{{AFX_MSG_MAP(CIDSGridCtrl)
	ON_WM_PAINT()
	ON_WM_HSCROLL()
	ON_WM_VSCROLL()
	ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CIDSGridCtrl message handlers

void CIDSGridCtrl::OnPaint()
{
...
BOOL CIDSGridCtrl::OnEraseBkgnd(CDC* /*pDC*/)
{
...
void CIDSGridCtrl::EraseBkgnd(CDC* pDC)
{
...
void CIDSGridCtrl::OnSize(UINT nType, int cx, int cy)
{  
...
UINT CIDSGridCtrl::OnGetDlgCode()
{
...
void CIDSGridCtrl::OnSysColorChange()
{
...
void CIDSGridCtrl::OnCaptureChanged(CWnd *pWnd)
{
...
void CIDSGridCtrl::OnSettingChange(UINT uFlags, LPCTSTR lpszSection)
{
...
void CIDSGridCtrl::OnTimer(UINT nIDEvent)
{
...
void CIDSGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
#if 1
...
#endif
	CIDSCellID next = m_idCurrentCell;
	...
	{
		CIDSCellID idOldTopLeft = GetTopleftNonFixedCell();
		SendMessage(WM_VSCROLL, SB_PAGEDOWN, 0);
		bVertScrollAction = TRUE;
		CIDSCellID idNewTopLeft = GetTopleftNonFixedCell();
...
void CIDSGridCtrl::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	CWnd::OnKeyUp(nChar, nRepCnt, nFlags);
}

void CIDSGridCtrl::OnSysKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
...
void CIDSGridCtrl::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	// EFW - BUG FIX
...
// Added by KiteFly
LRESULT CIDSGridCtrl::OnImeChar(WPARAM wCharCode, LPARAM)
{
	// EFW - BUG FIX
...
void CIDSGridCtrl::OnEndInPlaceEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	//InvalidateCellRect(CIDSCellID(pgvItem->row, pgvItem->col));
...
void CIDSGridCtrl::OnHScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
	EndEditing();
	...
	CIDSCellID idTopLeft = GetTopleftNonFixedCell();
	...
	case SB_THUMBTRACK:
		{
			int nScrollPos = GetScrollPos32(SB_HORZ, TRUE);
			SetScrollPos32(SB_HORZ, nScrollPos);
	...
	SendMessageToParent(idTopLeft.row, idTopLeft.col, WM_GRID_HSCOLL);
}

// Handle vert scrollbar notifications
void CIDSGridCtrl::OnVScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
	EndEditing();
	...
	CIDSCellID idTopLeft = GetTopleftNonFixedCell();
	int nRowIndex = idTopLeft.row;
	int nColIndex = idTopLeft.col;
	CRect rect;
	GetClientRect(rect);
	...
	SendMessageToParent(nRowIndex, nColIndex, WM_GRID_VSCOLL);
}


/////////////////////////////////////////////////////////////////////////////
// CIDSGridCtrl implementation functions

void CIDSGridCtrl::OnDraw(CDC* pDC)
{
	if (!m_bAllowDraw)
		return;
	...
	if(m_pBeforeDraw)
	{
		m_pBeforeDraw(pDC, m_dwBeforeDrawContext);
	}
#ifdef _DEBUG
	...
	CRect VisRect;
	CIDSCellRange VisCellRange = GetVisibleNonFixedCellRange(VisRect);
	...
	CPen penFiexedGrid;
	penFiexedGrid.CreatePen(PS_SOLID, 0, m_clFixtGridLine);

	pDC->SelectObject(&penFiexedGrid);
	...
	if (m_nGridLines == GVL_BOTH || m_nGridLines == GVL_VERT)
	{
		...
		int nFixedColWidthTemp = 0;
		for (int i = 0; i < m_nFixedCols; i++)
		{
			int nOneColWidth = GetColumnWidth(i);
			nFixedColWidthTemp += nOneColWidth;

			if (nOneColWidth <= 0)
			{
				continue;
			}
			pDC->MoveTo(nFixedColWidthTemp-1, 0);
			pDC->LineTo(nFixedColWidthTemp-1, VisRect.bottom);
		}
#endif
		...
		for (col = minVisibleCol; col <= maxVisibleCol; col++)
		{
			...
			pDC->SelectObject(&penFiexedGrid);
			pDC->MoveTo(x-1, 0);
			pDC->LineTo(x-1, nFixedRowHeight);
...
#if 1
		pDC->MoveTo(0, 0);
		pDC->LineTo(VisRect.right,  0);
		int nFixedRowHeightTemp = 0;
		for (int i = 0; i < m_nFixedRows; i++)
		{
			int nOneRowHeigt = GetRowHeight(i);
			nFixedRowHeightTemp += nOneRowHeigt;

			if (nOneRowHeigt <= 0)
			{
				continue;
			}
			pDC->MoveTo(0, nFixedRowHeightTemp-1);
			pDC->LineTo(VisRect.right,  nFixedRowHeightTemp-1);
		}
#endif
...
#endif
#else
	//	CPen lightpen(PS_SOLID, 1,  RGB(255,255,212)),
	//		darkpen(PS_SOLID,  1, RGB(222,183,77));
	CPen lightpen(PS_SOLID, 1,  RGB(0xff,0xfe,0xcd)),
		darkpen(PS_SOLID,  1, RGB(0xda,0xae,0x3e));
	...
	if (GetVirtualMode())
		SendCacheHintToParent(CIDSCellRange(-1,-1,-1,-1));
	...
	if(m_pAfterDraw)
	{
		m_pAfterDraw(pDC, m_dwAfterDrawContext);
	}
}

////////////////////////////////////////////////////////////////////////////////////////
// CIDSGridCtrl Cell selection stuff

// Is a given cell designation valid (ie within the bounds of our number
// of columns/rows)?
BOOL CIDSGridCtrl::IsValid(int nRow, int nCol) const
{
	return (nRow >= 0 && nRow < m_nRows && nCol >= 0 && nCol < m_nCols);
}

BOOL CIDSGridCtrl::IsValid(const CIDSCellID& cell) const
{
...
// of columns/rows)?
BOOL CIDSGridCtrl::IsValid(const CIDSCellRange& range) const
{
...
void CIDSGridCtrl::SetRedraw(BOOL bAllowDraw, BOOL bResetScrollBars /* = FALSE */)
{
...
// or the supplied dc)
BOOL CIDSGridCtrl::RedrawCell(const CIDSCellID& cell, CDC* pDC /* = NULL */)
{
	return RedrawCell(cell.row, cell.col, pDC);
}

BOOL CIDSGridCtrl::RedrawCell(int nRow, int nCol, CDC* pDC /* = NULL */)
{
	BOOL bResult = TRUE;
...
BOOL CIDSGridCtrl::RedrawRow(int row)
{
	BOOL bResult = TRUE;
...
BOOL CIDSGridCtrl::RedrawColumn(int col)
{
	BOOL bResult = TRUE;
...
CIDSCellID CIDSGridCtrl::SetFocusCell(int nRow, int nCol)
{
	return SetFocusCell(CIDSCellID(nRow, nCol));
}

CIDSCellID CIDSGridCtrl::SetFocusCell(CIDSCellID cell)
{
	...
	CIDSCellID idPrev = m_idCurrentCell;
	...
	// SendMessageToParent(cell.row, cell.col, WM_GRID_FOCUSCELL_CHANGE);
	return idPrev;
}

// Sets the range of currently selected cells
void CIDSGridCtrl::SetSelectedRange(const CIDSCellRange& Range,
...
void CIDSGridCtrl::SetSelectedRange(int nMinRow, int nMinCol, int nMaxRow, int nMaxCol,
									BOOL bForceRepaint /* = FALSE */, BOOL bSelectCells/*=TRUE*/)
{
	...
	CIDSCellRange VisCellRange, FixedVisCellRange;
	if (IsWindow(GetSafeHwnd()))
	...
	if (bSelectCells)
	{
			...
			nMinRow >= 0 && nMinCol >= 0 && nMaxRow >= 0 && nMaxCol >= 0)
		{
			for (pos = m_PrevSelectedCellMap.GetStartPosition(); pos != NULL; /* nothing */)
			{
				DWORD key;
				CIDSCellID cell;
				m_PrevSelectedCellMap.GetNextAssoc(pos, key, (CIDSCellID&)cell);
...
// selects all cells
void CIDSGridCtrl::SelectAllCells()
{
	if (!m_bEnableSelection)
		return;
	if(m_bFullRowSelected)
	{
		SetSelectedRange(0, 0, GetRowCount()-1, GetColumnCount()-1);
	}
	else
	{
...
// selects columns
void CIDSGridCtrl::SelectColumns(CIDSCellID currentCell, 
								 BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
	if (!m_bEnableSelection)
		return;

	if(FALSE == m_bEnableSelectCol)
	{
		return;
	}
...
// selects rows
void CIDSGridCtrl::SelectRows(CIDSCellID currentCell, 
							  BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
	if (!m_bEnableSelection)
		return;
	...
	if (!IsValid(currentCell))
		return;
	int Left = ( m_AllowSelectRowInFixedCol ? 0 : GetFixedColumnCount());
	if (GetSingleRowSelection() || m_bSingleRowSelection)
...
// selects cells
void CIDSGridCtrl::SelectCells(CIDSCellID currentCell, 
							   BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
	if (!m_bEnableSelection)
		return;
...
void CIDSGridCtrl::OnSelecting(const CIDSCellID& currentCell)
{
	if (!m_bEnableSelection)
		return;
...
void CIDSGridCtrl::ValidateAndModifyCellContents(int nRow, int nCol, LPCTSTR strText)
{
...
void CIDSGridCtrl::ClearCells(CIDSCellRange Selection)
{
...
void CIDSGridCtrl::CutSelectedText()
{
	if (!IsEditable())
		return;
...
COleDataSource* CIDSGridCtrl::CopyTextFromGrid()
{
	USES_CONVERSION;

	CIDSCellRange Selection = GetSelectedCellRange();
...
BOOL CIDSGridCtrl::PasteTextToGrid(CIDSCellID cell, COleDataObject* pDataObject, 
								   BOOL bSelectPastedCells /*=TRUE*/)
{
	...
	CIDSCellRange PasteRange(cell.row, cell.col,-1,-1);
	int nIndex;
	do
	{
		int nColumn = 0;
		...
		while (!strLine.IsEmpty())
		{
			...
			CIDSCellID TargetCell(iRowVis, iColVis);
...
DROPEFFECT CIDSGridCtrl::OnDragEnter(COleDataObject* pDataObject, DWORD dwKeyState,
									 CPoint point)
{   
...
BOOL CIDSGridCtrl::OnDrop(COleDataObject* pDataObject, DROPEFFECT /*dropEffect*/,
						  CPoint /* point */)
{
	m_MouseMode = MOUSE_NOTHING;
	if(m_CurCol ==-1)
...
void CIDSGridCtrl::OnEditPaste()
{
	if (!IsEditable())
		return;

	CIDSCellRange cellRange = GetSelectedCellRange();
	...
	{
		CIDSGridCellBase* pCell = GetCell(cell.row, cell.col);
		ASSERT(pCell);
		if (!pCell) return;
...
void CIDSGridCtrl::OnEditSelectAll()
{
...
#ifndef GRIDCONTROL_NO_CLIPBOARD
void CIDSGridCtrl::OnUpdateEditCopy(CCmdUI* pCmdUI)
{
...
void CIDSGridCtrl::OnUpdateEditCut(CCmdUI* pCmdUI)
{
...
void CIDSGridCtrl::OnUpdateEditPaste(CCmdUI* pCmdUI)
{
	CIDSCellID cell = GetFocusCell();
...
void CIDSGridCtrl::OnUpdateEditSelectAll(CCmdUI* pCmdUI)
{
...
BOOL CIDSGridCtrl::MouseOverRowResizeArea(CPoint& point)
{
	if (point.x >= GetFixedColumnWidth())
		return FALSE;

	CIDSCellID idCurrentCell = GetCellFromPt(point);
	CPoint start;
...
BOOL CIDSGridCtrl::MouseOverColumnResizeArea(CPoint& point)
{
...
CIDSCellID CIDSGridCtrl::GetCellFromPt(CPoint point, BOOL bAllowFixedCellCheck /*=TRUE*/)
{
	CIDSCellID cellID; // return value
...
////////////////////////////////////////////////////////////////////////////////
// CIDSGridCtrl cellrange functions

// Gets the first non-fixed cell ID
CIDSCellID CIDSGridCtrl::GetTopleftNonFixedCell(BOOL bForceRecalculation /*=FALSE*/)
{
...
CIDSCellRange CIDSGridCtrl::GetVisibleNonFixedCellRange(LPRECT pRect /*=NULL*/, 
														BOOL bForceRecalculation /*=FALSE*/)
{
	...
	CIDSCellID idTopLeft = GetTopleftNonFixedCell(bForceRecalculation);
	...
	return CIDSCellRange(idTopLeft.row, idTopLeft.col, maxVisibleRow, maxVisibleCol);
}


CIDSCellRange CIDSGridCtrl::GetVisibleFixedCellRange(LPRECT pRect /*=NULL*/, 
													 BOOL bForceRecalculation /*=FALSE*/)
{
	int i;
	CRect rect;
	...
	return CIDSCellRange(idTopLeft.row, 0, maxVisibleRow, maxVisibleCol);
}
...
CIDSCellRange CIDSGridCtrl::GetUnobstructedNonFixedCellRange(BOOL bForceRecalculation /*=FALSE*/)
{
	CRect rect;
	GetClientRect(rect);
...
// If no selection, then the returned CIDSCellRange will be invalid
CIDSCellRange CIDSGridCtrl::GetSelectedCellRange() const
{
	CIDSCellRange Selection(GetRowCount(), GetColumnCount(), -1,-1);
...
CIDSCellRange CIDSGridCtrl::GetCellRange() const
{
	return CIDSCellRange(0, 0, GetRowCount() - 1, GetColumnCount() - 1);
}

// Resets the selected cell range to the empty set.
void CIDSGridCtrl::ResetSelectedRange()
{
	m_PrevSelectedCellMap.RemoveAll();
...
int CIDSGridCtrl::GetScrollPos32(int nBar, BOOL bGetTrackPos /* = FALSE */)
{
	SCROLLINFO si;
...
BOOL CIDSGridCtrl::SetScrollPos32(int nBar, int nPos, BOOL bRedraw /* = TRUE */)
{
	m_idTopLeftCell.row = -1;
...
void CIDSGridCtrl::EnableScrollBars(int nBar, BOOL bEnable /*=TRUE*/)
{
	if(FALSE == m_bAutoShowScrollBar)
	{
		return;
	}
	if (bEnable)
...
void CIDSGridCtrl::ResetScrollBars()
{
	...
	if (IsVisibleVScroll() && m_bHasVScrollBar)
	{
		rect.right += GetSystemMetrics(SM_CXVSCROLL) + GetSystemMetrics(SM_CXBORDER);
	}

	if (IsVisibleHScroll() && m_bHasHScrollBar)
	{
	...
	// CIDSCellRange visibleCells = GetUnobstructedNonFixedCellRange();
...
BOOL CIDSGridCtrl::GetCellOrigin(int nRow, int nCol, LPPOINT p)
{
	int i;
...
BOOL CIDSGridCtrl::GetCellOrigin(const CIDSCellID& cell, LPPOINT p)
{
	return GetCellOrigin(cell.row, cell.col, p);
}

// Returns the bounding box of the cell
BOOL CIDSGridCtrl::GetCellRect(const CIDSCellID& cell, LPRECT pRect)
{
	return GetCellRect(cell.row, cell.col, pRect);
}

BOOL CIDSGridCtrl::GetCellRect(int nRow, int nCol, LPRECT pRect)
{
	CPoint CellOrigin;
...
BOOL CIDSGridCtrl::GetTextRect(const CIDSCellID& cell, LPRECT pRect)
{
	return GetTextRect(cell.row, cell.col, pRect);
}

BOOL CIDSGridCtrl::GetTextRect(int nRow, int nCol, LPRECT pRect)
{
	CIDSGridCellBase* pCell = GetCell( nRow, nCol);
	if( pCell == NULL)
		return FALSE;
...
BOOL CIDSGridCtrl::GetCellRangeRect(const CIDSCellRange& cellRange, LPRECT lpRect)
{
...
LRESULT CIDSGridCtrl::OnSetFont(WPARAM hFont, LPARAM /*lParam */)
{
...
LRESULT CIDSGridCtrl::OnGetFont(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	//LOGFONT    lf;
...
#ifndef _WIN32_WCE_NO_CURSOR
BOOL CIDSGridCtrl::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	if (nHitTest == HTCLIENT)
	{
		...
		default:
			if (!GetVirtualMode())
			{
				...
				ScreenToClient(&pt);
				CIDSCellID cell = GetCellFromPt(pt);
				if (IsValid(cell))
				{
					CIDSGridCellBase* pCell = GetCell(cell.row, cell.col);
					if (pCell)
...
BOOL CIDSGridCtrl::SetFixedRowCount(int nFixedRows)
{
...
BOOL CIDSGridCtrl::SetFixedColumnCount(int nFixedCols)
{
...
BOOL CIDSGridCtrl::SetRowCount(int nRows)
{
	BOOL bResult = TRUE;
...
BOOL CIDSGridCtrl::SetColumnCount(int nCols)
{
	BOOL bResult = TRUE;
...
int CIDSGridCtrl::InsertColumn(LPCTSTR strHeading,
...
int CIDSGridCtrl::InsertRow(LPCTSTR strHeading, int nRow /* = -1 */)
{
...
BOOL CIDSGridCtrl::SetCellType(int nRow, int nCol, CRuntimeClass* pRuntimeClass)
{
	if (GetVirtualMode())
		return FALSE;
	...
	if (!pRuntimeClass->IsDerivedFrom(RUNTIME_CLASS(CIDSGridCellBase)))
	{
		ASSERT( FALSE);
		return FALSE;
	}

	CIDSGridCellBase* pNewCell = (CIDSGridCellBase*) pRuntimeClass->CreateObject();

	CIDSGridCellBase* pCurrCell = GetCell(nRow, nCol);
	if (pCurrCell)
		*pNewCell = *pCurrCell;
...
BOOL CIDSGridCtrl::SetDefaultCellType( CRuntimeClass* pRuntimeClass)
{
...
/*virtual*/ CIDSGridCellBase* CIDSGridCtrl::CreateCell(int nRow, int nCol)
{
	ASSERT(!GetVirtualMode());

	if (!m_pRtcDefault || !m_pRtcDefault->IsDerivedFrom(RUNTIME_CLASS(CIDSGridCellBase)))
	{
		ASSERT( FALSE);
		return NULL;
	}
	CIDSGridCellBase* pCell = (CIDSGridCellBase*) m_pRtcDefault->CreateObject();
	if (!pCell)
		return NULL;
...
/*virtual*/ void CIDSGridCtrl::DestroyCell(int nRow, int nCol)
{
...
BOOL CIDSGridCtrl::DeleteColumn(int nColumn)
{
	if (nColumn < 0 || nColumn >= GetColumnCount())
		return FALSE;
...
void CIDSGridCtrl::AddSubVirtualRow(int Num, int Nb)
{
...
BOOL CIDSGridCtrl::DeleteRow(int nRow)
{
	if (nRow < 0 || nRow >= GetRowCount())
		return FALSE;
...
BOOL CIDSGridCtrl::DeleteNonFixedRows()
{
	ResetSelectedRange();
...
BOOL CIDSGridCtrl::DeleteAllItems()
{
	if(m_InDestructor)
	{
	}
...
/////////////////////////////////////////////////////////////////////////////
// CIDSGridCtrl data functions

// Set CListCtrl::GetNextItem for details
CIDSCellID CIDSGridCtrl::GetNextItem(CIDSCellID& cell, int nFlags) const
{
	...
	else if ((nFlags & GVNI_ABOVE) == GVNI_ABOVE) 
	{
		for (int row = cell.row - 1; row >= GetFixedRowCount(); row--) 
		{
			...
			if ((nFlags & GVNI_DROPHILITED && nState & GVIS_DROPHILITED) || 
				...
				(nFlags & GVNI_FIXED       && nState & GVIS_FIXED)       ||
				(nFlags & GVNI_MODIFIED    && nState & GVIS_MODIFIED))
				return CIDSCellID(row, cell.col);
		}
	}
	else if ((nFlags & GVNI_BELOW) == GVNI_BELOW)
	{
	...
	else if ((nFlags & GVNI_TOLEFT) == GVNI_TOLEFT)
	{
		for (int col = cell.col - 1; col >= GetFixedColumnCount(); col--) 
		{
			...
			if ((nFlags & GVNI_DROPHILITED && nState & GVIS_DROPHILITED) || 
				...
				(nFlags & GVNI_FIXED       && nState & GVIS_FIXED)       ||
				(nFlags & GVNI_MODIFIED    && nState & GVIS_MODIFIED))
				return CIDSCellID(cell.row, col);
		}
	}
	...
	return CIDSCellID(-1, -1);
}

// Sorts on a given column using the cell text
BOOL CIDSGridCtrl::SortTextItems(int nCol, BOOL bAscending, LPARAM data /* = 0 */)
{
	return CIDSGridCtrl::SortItems(pfnCellTextCompare, nCol, bAscending, data);
}

void CIDSGridCtrl::SetCompareFunction(PFNLVCOMPARE pfnCompare,  DWORD dwSortContext)
{
	m_pfnCompare = pfnCompare;
	m_dwSortContext = dwSortContext;
}

// Sorts on a given column using the cell text and using the specified comparison
// function
BOOL CIDSGridCtrl::SortItems(int nCol, BOOL bAscending, LPARAM data /* = 0 */)
{
	SetSortColumn(nCol);
	...
	{
		return m_pfnCompare(nCol, bAscending, m_dwSortContext);
		//return CIDSGridCtrl::SortItems(m_pfnCompare, nCol, bAscending, m_dwSortContext);
	}
}

// Sorts on a given column using the supplied compare function (see CListCtrl::SortItems)
BOOL CIDSGridCtrl::SortItems(PFNLVCOMPARE pfnCompare, int nCol, BOOL bAscending,
							 LPARAM data /* = 0 */)
{
	SetSortColumn(nCol);
	SetSortAscending(bAscending);
...
int CALLBACK CIDSGridCtrl::pfnCellTextCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	UNUSED_ALWAYS(lParamSort);

	CIDSGridCellBase* pCell1 = (CIDSGridCellBase*) lParam1;
	CIDSGridCellBase* pCell2 = (CIDSGridCellBase*) lParam2;
	if (!pCell1 || !pCell2) return 0;

	return _tcscmp(pCell1->GetText(), pCell2->GetText());
}

int CALLBACK CIDSGridCtrl::pfnCellNumericCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	UNUSED_ALWAYS(lParamSort);
...
CIDSGridCtrl *  CIDSGridCtrl::m_This;
// private recursive sort implementation
bool CIDSGridCtrl::NotVirtualCompare(int c1, int c2)
{
	return ! CIDSGridCtrl::m_This->m_pfnVirtualCompare(c1, c2);
}
BOOL CIDSGridCtrl::SortItems(PFNLVCOMPARE pfnCompare, int nCol, BOOL bAscending, LPARAM data,
							 int low, int high)
{
	...
	while (lo <= hi)
	{
		...
		if (lo <= hi)
		{
			...
			if (pfnCompare((LPARAM)GetCell(lo, nCol), (LPARAM)GetCell(hi, nCol), data) != 0)
			{
				for (int col = 0; col < GetColumnCount(); col++)
				{
					CIDSGridCellBase *pCell = GetCell(lo, col);
...
BOOL CIDSGridCtrl::SetItem(const GV_ITEM* pItem)
{
	if (!pItem || GetVirtualMode())
		return FALSE;

	CIDSGridCellBase* pCell = GetCell(pItem->row, pItem->col);
	if (!pCell)
		return FALSE;
...
BOOL CIDSGridCtrl::GetItem(GV_ITEM* pItem)
{
	if (!pItem)
		return FALSE;
	CIDSGridCellBase* pCell = GetCell(pItem->row, pItem->col);
...
BOOL CIDSGridCtrl::SetItemText(int nRow, int nCol, LPCTSTR str)
{
	if (GetVirtualMode())
		return FALSE;
...
BOOL CIDSGridCtrl::SetItemTextFmt(int nRow, int nCol, LPCTSTR szFmt, ...)
{
	if (GetVirtualMode())
		return FALSE;
...
BOOL CIDSGridCtrl::SetItemTextFmtID(int nRow, int nCol, UINT nID, ...)
{
	if (GetVirtualMode())
		return FALSE;
...
BOOL CIDSGridCtrl::SetItemData(int nRow, int nCol, LPARAM lParam)
{
	if (GetVirtualMode())
		return FALSE;
...
LPARAM CIDSGridCtrl::GetItemData(int nRow, int nCol) const
{
	CIDSGridCellBase* pCell = GetCell(nRow, nCol);
	if (!pCell)
		return (LPARAM) 0;
...
BOOL CIDSGridCtrl::SetItemImage(int nRow, int nCol, int iImage)
{
	if (GetVirtualMode())
		return FALSE;
...
int CIDSGridCtrl::GetItemImage(int nRow, int nCol) const
{
	CIDSGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if (!pCell)
...
BOOL CIDSGridCtrl::SetItemState(int nRow, int nCol, UINT state)
{
	...
	if (bSelected && !(state & GVIS_SELECTED))
	{
		...
		if (m_SelectedCellMap.Lookup(key, (CIDSCellID&)cell))
...
UINT CIDSGridCtrl::GetItemState(int nRow, int nCol) const
{
	CIDSGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if (!pCell)
...
BOOL CIDSGridCtrl::SetItemFormat(int nRow, int nCol, UINT nFormat)
{
	if (GetVirtualMode())
		return FALSE;
...
UINT CIDSGridCtrl::GetItemFormat(int nRow, int nCol) const
{
	CIDSGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if (!pCell)
...
BOOL CIDSGridCtrl::SetItemBkColour(int nRow, int nCol, COLORREF cr /* = CLR_DEFAULT */)
{
	if (GetVirtualMode())
		return FALSE;
...
COLORREF CIDSGridCtrl::GetItemBkColour(int nRow, int nCol) const
{
	CIDSGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if (!pCell)
...
BOOL CIDSGridCtrl::SetItemFgColour(int nRow, int nCol, COLORREF cr /* = CLR_DEFAULT */)
{
	if (GetVirtualMode())
		return FALSE;
...
COLORREF CIDSGridCtrl::GetItemFgColour(int nRow, int nCol) const
{
	CIDSGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if (!pCell)
...
BOOL CIDSGridCtrl::SetItemFont(int nRow, int nCol, const LOGFONT* plf)
{
	if (GetVirtualMode())
		return FALSE;
...
BOOL CIDSGridCtrl::SetItemFont(int nRow ,int nCol, int nFontHeight, LPCTSTR pszFontName, BOOL bBold) 
{
	if (GetVirtualMode())
		return FALSE;
	...
	_tcscpy(lf.lfFaceName, pszFontName);
	if(bBold)
	{
...
const LOGFONT* CIDSGridCtrl::GetItemFont(int nRow, int nCol)
{
	CIDSGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if (!pCell) 
...
BOOL CIDSGridCtrl::IsItemEditing(int nRow, int nCol)
{
	CIDSGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if (!pCell)
...
long CIDSGridCtrl::GetVirtualWidth() const
{
...
long CIDSGridCtrl::GetVirtualHeight() const
{
...
int CIDSGridCtrl::GetRowHeight(int nRow) const
{
...
int CIDSGridCtrl::GetColumnWidth(int nCol) const
{
...
BOOL CIDSGridCtrl::SetRowHeight(int nRow, int height, BOOL bResetScrollBars)
{
...
BOOL CIDSGridCtrl::SetColumnWidth(int nCol, int width, BOOL bResetScrollBars)
{
...
int CIDSGridCtrl::GetFixedRowHeight() const
{
	int nHeight = 0;
	for (int i = 0; i < m_nFixedRows; i++)
...
int CIDSGridCtrl::GetFixedColumnWidth() const
{
	int nWidth = 0;
	for (int i = 0; i < m_nFixedCols; i++)
...
BOOL CIDSGridCtrl::AutoSizeColumn(int nCol, UINT nAutoSizeStyle /*=GVS_DEFAULT*/, 
								  BOOL bResetScroll /*=TRUE*/)
{
	...
	if (GetVirtualMode())
		SendCacheHintToParent(CIDSCellRange(nStartRow, nCol, nEndRow, nCol));
...
BOOL CIDSGridCtrl::AutoSizeRow(int nRow, BOOL bResetScroll /*=TRUE*/)
{
	ASSERT(nRow >= 0 && nRow < m_nRows);
	if (nRow < 0 || nRow >= m_nRows)
		return FALSE;
	...
	if (GetVirtualMode())
		SendCacheHintToParent(CIDSCellRange(nRow, 0, nRow, nNumColumns));
...
void CIDSGridCtrl::AutoSizeColumns(UINT nAutoSizeStyle /*=GVS_DEFAULT*/)
{
	int nNumColumns = GetColumnCount();
	for (int nCol = 0; nCol < nNumColumns; nCol++)
	{
...
void CIDSGridCtrl::AutoSizeRows()
{
...
void CIDSGridCtrl::AutoSize(UINT nAutoSizeStyle /*=GVS_DEFAULT*/)
{
	CDC* pDC = GetDC();
	if (!pDC)
		return;
	...
	if (GetVirtualMode())
		SendCacheHintToParent(CIDSCellRange(nStartRow, 0, nEndRow, nNumColumns));
...
void CIDSGridCtrl::ExpandColumnsToFit(BOOL bExpandFixed /*=TRUE*/)
{
	if (bExpandFixed)
	{
...
void CIDSGridCtrl::ExpandLastColumn()
{
	if (GetColumnCount() <= 0)
		return;
...
void CIDSGridCtrl::ExpandRowsToFit(BOOL bExpandFixed /*=TRUE*/)
{
	if (bExpandFixed)
	{
...
void CIDSGridCtrl::ExpandToFit(BOOL bExpandFixed /*=TRUE*/)
{
...
void CIDSGridCtrl::SetVirtualMode(BOOL bVirtual)
{
	DeleteAllItems();
...
void CIDSGridCtrl::SetGridLines(int nWhichLines /*=GVL_BOTH*/) 
{
	m_nGridLines = nWhichLines;
	Refresh();
}

void CIDSGridCtrl::SetListMode(BOOL bEnableListMode /*=TRUE*/)
{
	ResetSelectedRange();
	SetSortColumn(-1);
...
void CIDSGridCtrl::SetSortColumn(int nCol)
{
	if (m_nSortColumn >= 0)
...
BOOL CIDSGridCtrl::IsCellFixed(int nRow, int nCol)
{
	return (nRow < GetFixedRowCount() || nCol < GetFixedColumnCount());
}

void CIDSGridCtrl::SetModified(BOOL bModified /*=TRUE*/, int nRow /*=-1*/, int nCol /*=-1*/)
{
...
BOOL CIDSGridCtrl::GetModified(int nRow /*=-1*/, int nCol /*=-1*/)
{
	if (nRow >= 0 && nCol >= 0)
...
void CIDSGridCtrl::EnsureVisible(int nRow, int nCol)
{
	if (!m_bAllowDraw)
		return;
	...
	CIDSCellID idTopLeft = GetTopleftNonFixedCell(FALSE);
	CIDSCellID idNewTopLeft = GetTopleftNonFixedCell(TRUE);
	if (idNewTopLeft != idTopLeft)
	{
	...
	*/
	...
	CIDSCellRange VisibleCells = GetVisibleNonFixedCellRange();
...
BOOL CIDSGridCtrl::IsCellEditable(CIDSCellID &cell) const
{
	return IsCellEditable(cell.row, cell.col);
}

BOOL CIDSGridCtrl::IsCellEditable(int nRow, int nCol) const
{
	return IsEditable() && ((GetItemState(nRow, nCol) & GVIS_READONLY) != GVIS_READONLY);
}

BOOL CIDSGridCtrl::IsCellSelected(CIDSCellID &cell) const
{
	return IsCellSelected(cell.row, cell.col);
}

BOOL CIDSGridCtrl::IsCellSelected(int nRow, int nCol) const
{
	if (GetVirtualMode())
	{   
		if (!IsSelectable())
		...
		return (m_SelectedCellMap.Lookup(key, (CIDSCellID&)cell));       
	}
	else
		return IsSelectable() && ((GetItemState(nRow, nCol) & GVIS_SELECTED) == GVIS_SELECTED);
}

BOOL CIDSGridCtrl::IsCellVisible(CIDSCellID cell) 
{
	return IsCellVisible(cell.row, cell.col);
}

BOOL CIDSGridCtrl::IsCellVisible(int nRow, int nCol)
{
	if (!IsWindow(m_hWnd))
		return FALSE;
...
BOOL CIDSGridCtrl::InvalidateCellRect(const CIDSCellID& cell)
{
	return InvalidateCellRect(cell.row, cell.col);
}

BOOL CIDSGridCtrl::InvalidateCellRect(const int row, const int col)
{
...
BOOL CIDSGridCtrl::InvalidateCellRect(const CIDSCellRange& cellRange)
{
	ASSERT(IsValid(cellRange));
	if (!::IsWindow(GetSafeHwnd()) || !m_bAllowDraw)
		return FALSE;

	CIDSCellRange visibleCellRange = GetVisibleNonFixedCellRange().Intersect(cellRange);
...
void CIDSGridCtrl::OnMouseMove(UINT /*nFlags*/, CPoint point)
{   
	CRect rect;
	GetClientRect(rect);
		...
		(m_bLMouseButtonDown && m_MouseMode == MOUSE_NOTHING))
	{
		...
		if (m_MouseMode == MOUSE_NOTHING)
		{
			CIDSGridCellBase* pCell = NULL;
			CIDSCellID idCurrentCell;
...
CPoint CIDSGridCtrl::GetPointClicked(int nRow, int nCol, const CPoint& point)
{
...
void CIDSGridCtrl::OnLButtonDblClk(UINT nFlags, CPoint point)
{   

	TRACE0("CIDSGridCtrl::OnLButtonDblClk\n");

	CIDSCellID cell = GetCellFromPt(point);
	if( !IsValid( cell) )
	{
		//ASSERT(FALSE);
		return;
	...
	SetFocusCell(cell); //鼠标按下,重置焦点单元格
#ifdef _WIN32_WCE
...
void CIDSGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
#ifdef GRIDCONTROL_USE_TITLETIPS
	// EFW - Bug Fix
	m_TitleTip.Hide();  // hide any titletips
	...
	// TRACE0("CIDSGridCtrl::OnLButtonDown\n");
	...
	if (!IsValid(m_LeftClickDownCell))
	{//点击非表格区域,则将焦点窗口设置为 无效
		SetFocusCell(-1, -1); 
		return;
	}
	m_CurRow = m_LeftClickDownCell.row;

	SetFocusCell(m_LeftClickDownCell); //鼠标按下,重置焦点单元格
	...
	CIDSGridCellBase* pCell = GetCell(m_LeftClickDownCell.row, m_LeftClickDownCell.col);
	if (pCell)
	...
	else if (m_MouseMode == MOUSE_OVER_ROW_DIVIDE) // sizing row
	{
...
#endif
		{
			...
			if (m_LeftClickDownCell.row < GetFixedRowCount())
			{
				//OnFixedRowClick(m_LeftClickDownCell);			//  Notes By Carrot 20150617  标题拖动
...
//					ResetSelectedRange(); // TODO : This is not the better solution, as we do not see why clicking in column header should reset selection
		...
		SendMessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col, WM_RGRIDLBUTTONDOWN);
}

void CIDSGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
	// TRACE0("CIDSGridCtrl::OnLButtonUp\n");
	...
	{
		CIDSGridCellBase* pCell = GetCell(m_idCurrentCell.row, m_idCurrentCell.col);
		if (pCell)
...
//		ResetSelectedRange();
	...
	SendMessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col, WM_RGRIDLBUTTONUP);
	if (!IsValid(m_LeftClickDownCell))
		return;
...
#ifndef _WIN32_WCE
//chris
void CIDSGridCtrl::OnRButtonDown(UINT nFlags, CPoint point)
{   
	...
	CIDSCellID FocusCell;
	FocusCell = GetCellFromPt(point);

	SendMessageToParent(FocusCell.row, FocusCell.col, WM_RGRIDRBUTTONDOWN);
}

// EFW - Added to forward right click to parent so that a context
// menu can be shown without deriving a new grid class.
void CIDSGridCtrl::OnRButtonUp(UINT nFlags, CPoint point)
{
	CWnd::OnRButtonUp(nFlags, point);

	//SendMessageToParent(m_LeftClickDownCell.row, m_LeftClickDownCell.col, WM_RGRIDBUTTONUP);
	...
	else
	{
		...
		// tell the cell about it
		CIDSGridCellBase* pCell = GetCell(FocusCell.row, FocusCell.col);
		if (pCell)
		...
		//pOwner->SendMessage(NM_RCLICK,(WPARAM) 0, (LPARAM) 0);
	}

	SendMessageToParent(FocusCell.row, FocusCell.col, WM_RGRIDRBUTTONUP);
}
#endif

#if !defined(_WIN32_WCE_NO_PRINTING) && !defined(GRIDCONTROL_NO_PRINTING)
/////////////////////////////////////////////////////////////////////////////
// CIDSGridCtrl printing

// EFW - New print margin support functions
void CIDSGridCtrl::SetPrintMarginInfo(int nHeaderHeight, int nFooterHeight,
...
void CIDSGridCtrl::GetPrintMarginInfo(int &nHeaderHeight, int &nFooterHeight,
									  int &nLeftMargin, int &nRightMargin, int &nTopMargin,
									  int &nBottomMargin, int &nGap)
{
...
void CIDSGridCtrl::Print(CPrintDialog* pPrntDialog /*=NULL*/)
{
	CDC dc;
...
/////////////////////////////////////////////////////////////////////////////
// CIDSGridCtrl printing overridables - for Doc/View print/print preview framework

// EFW - Various changes in the next few functions to support the
// new print margins and a few other adjustments.
void CIDSGridCtrl::OnBeginPrinting(CDC *pDC, CPrintInfo *pInfo)
{
	...
	/*   m_PrinterFont.CreateFont(nFontSize, 0,0,0, FW_ULTRABOLD, 0,0,0, DEFAULT_CHARSET,
...
void CIDSGridCtrl::OnPrint(CDC *pDC, CPrintInfo *pInfo)
{
	if (!pDC || !pInfo)
		return;
	...
	while (m_nCurrPrintRow < GetRowCount())
	{
		...
		for (int col = m_nPrintColumn; col < GetColumnCount(); col++)
		{
			...
			CIDSGridCellBase* pCell = GetCell(m_nCurrPrintRow, col);
			if (pCell)
...
void CIDSGridCtrl::PrintFixedRowCells(int nStartColumn, int nStopColumn, int& row, CRect& rect,
									  CDC *pDC, BOOL& bFirst)
{
	...
	for (int col =nStartColumn; col < nStopColumn; col++)
	{
		...
		CIDSGridCellBase* pCell = GetCell(row, col);
		if (pCell)
...
} // end of CIDSGridCtrl::PrintFixedRowCells

void CIDSGridCtrl::PrintColumnHeadings(CDC *pDC, CPrintInfo* /*pInfo*/)
{
	CFont *pOldFont = pDC->SelectObject(&m_PrinterFont);
	...
	pDC->SelectObject(pOldFont);
} // end of CIDSGridCtrl::PrintColumnHeadings
...
*****************************************************************************/
void CIDSGridCtrl::PrintRowButtons(CDC *pDC, CPrintInfo* /*pInfo*/)
{
	CFont *pOldFont = pDC->SelectObject(&m_PrinterFont);
	...
	for( int iCol = 0; iCol < GetFixedColumnCount(); iCol++)
	{
		...
		for( int iRow = m_nCurrPrintRow; iRow < GetRowCount(); iRow++)
		{
			...
			CIDSGridCellBase* pCell = GetCell(iRow, iCol);
			if (pCell)
...
void CIDSGridCtrl::PrintHeader(CDC *pDC, CPrintInfo *pInfo)
{
...
void CIDSGridCtrl::PrintFooter(CDC *pDC, CPrintInfo *pInfo)
{
...
void CIDSGridCtrl::OnEndPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
...
#ifndef _WIN32_WCE
/////////////////////////////////////////////////////////////////////////////
// CIDSGridCtrl persistance

BOOL CIDSGridCtrl::Save(LPCTSTR filename, TCHAR chSeparator/*=_T(',')*/)
{
	CStdioFile File;
	CFileException ex;
...
BOOL CIDSGridCtrl::Load(LPCTSTR filename, TCHAR chSeparator/*=_T(',')*/)
{
	if (GetVirtualMode())
		return FALSE;
...
/////////////////////////////////////////////////////////////////////////////
// CIDSGridCtrl overrideables

#ifndef GRIDCONTROL_NO_DRAGDROP
// This is no longer needed since I've changed to OLE drag and drop - but it's
// still cool code. :)
CImageList* CIDSGridCtrl::CreateDragImage(CPoint *pHotSpot)
{
	CDC* pDC = GetDC();
	if (!pDC)
		return NULL;
...
void CIDSGridCtrl::OnFixedColumnClick(CIDSCellID& cell)
{
	if (!IsValid(cell))
		return;
...
CSize CIDSGridCtrl::GetTextExtent(int nRow, int nCol, LPCTSTR str)
{
	CIDSGridCellBase* pCell = GetCell(nRow, nCol);
	if (!pCell)
		return CSize(0, 0);
...
// virtual
void CIDSGridCtrl::OnEditCell(int nRow, int nCol, CPoint point, UINT nChar)
{
	return;//@add by wu , no editable
#ifndef GRIDCONTROL_NO_TITLETIPS
	m_TitleTip.Hide();  // hide any titletips
#endif
...
// virtual
void CIDSGridCtrl::OnEndEditCell(int nRow, int nCol, CString str)
{
	CString strCurrentText = GetItemText(nRow, nCol);
...
// virtual
BOOL CIDSGridCtrl::ValidateEdit(int nRow, int nCol, LPCTSTR str)
{
	CIDSGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if (!pCell)
...
// virtual
CString CIDSGridCtrl::GetItemText(int nRow, int nCol) const
{
...
void CIDSGridCtrl::ResetVirtualOrder()
{
...
void CIDSGridCtrl::SetRowFullSelect(BOOL bFullSelect)
{
	m_bFullRowSelected = bFullSelect;
}

BOOL CIDSGridCtrl::IsDrawGridLine()
{
	return m_bDrawGridLine;	
}
void CIDSGridCtrl::SetFixedGridLineColor(COLORREF clFixtGridLine)
{
	m_clFixtGridLine = clFixtGridLine;
}
COLORREF CIDSGridCtrl::GetFixtedGridLineColor()
{
	return m_clFixtGridLine;
}

void CIDSGridCtrl::SetSelectItemBKColor(COLORREF clSelBK)
{
	m_clSelectBK = clSelBK;
}
COLORREF CIDSGridCtrl::GetSelectItemBKColor()
{
	return m_clSelectBK;
}

void CIDSGridCtrl::SetDrawGridLine(BOOL bDrawGridLine)
{
	m_bDrawGridLine = bDrawGridLine;
}	


void CIDSGridCtrl::SetHasHScrollBar(BOOL bHasHScrollBar)
{
	m_bHasHScrollBar = bHasHScrollBar;
}

void CIDSGridCtrl::SetHasVScroolBar(BOOL bHasVScrollBar)
{
	m_bHasVScrollBar = bHasVScrollBar;
}

void CIDSGridCtrl::SetAutoShowScrollBar(BOOL bAutoShowScrollBar)
{ 
	m_bAutoShowScrollBar = bAutoShowScrollBar;
}
void CIDSGridCtrl::SetDrawFocusCell(BOOL bDrawFocusCell)
{
	m_bDrawFocusCell = bDrawFocusCell;
}
BOOL CIDSGridCtrl::IsDrawFocusCell()
{
	return m_bDrawFocusCell;
}

void CIDSGridCtrl::ResetCustomize()
{
	m_bFullRowSelected = FALSE;
	m_bDrawGridLine = TRUE;
	m_clFixtGridLine = 0;
	m_pBeforeDraw = NULL;
	m_dwBeforeDrawContext = 0;
	m_pAfterDraw = NULL;
	m_dwAfterDrawContext = NULL;
	m_dwSortContext = 0;
	m_bAutoShowScrollBar = TRUE;
	m_bHasHScrollBar = TRUE;
	m_bHasVScrollBar = TRUE;
...
{
	m_pBeforeDraw = pBeforeDrawFun;
	m_dwBeforeDrawContext = dwBeforeContext;
}
void CIDSGridCtrl::SetAfterDrawFunc(UserDraw pAfterDrawFun, DWORD dwAfterContext)\
{
	m_pAfterDraw = pAfterDrawFun;
	m_dwAfterDrawContext = dwAfterContext;
}


BOOL CIDSGridCtrl::SetSelectRow(int nRow)
{
	if(-1 == nRow)
	{
		this->SetFocusCell(-1, -1);
		this->SetSelectedRange(0,0, -1, -1);
		return TRUE;
	}
	...
	cCurCell.row = nRow;
	cCurCell.col = 0;
	this->SetFocusCell(cCurCell);
	this->SelectRows(cCurCell);
	return TRUE;
}

void CIDSGridCtrl::ScrollBarToZero(BOOL bNeedResetScrollBar)
{
	if(bNeedResetScrollBar)
	{
		this->ResetScrollBars();
	}
	this->SetScrollPos32(SB_HORZ, 0, FALSE);
	this->SetScrollPos32(SB_VERT, 0, TRUE);
}

void CIDSGridCtrl::SetGridFont(LPCTSTR pszFontName, int nFontHeight, int nFontWeight)
{
	if(NULL == pszFontName || 0  ==_tcslen(pszFontName) )
	{ 
	}
	else
	{
		_tcscpy(g_tFontInfo.szFontName, pszFontName);
	}

	if(nFontHeight != 0)
	{
		g_tFontInfo.nFontHeight = nFontHeight;
	}
	if(nFontWeight != 0)
	{
		g_tFontInfo.nFontWight = nFontWeight;		
	}
}